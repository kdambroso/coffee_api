class Drink
    ...
    def self.all
        result = DB.exec("SELECT * FROM coffees;")
    ...
    def self.find(id)
        results = DB.exec("SELECT * FROM coffees WHERE id=#{id};")
        return {
    ...
    def self.create(opts)
        results = DB.exec(
            <<-SQL
                INSERT INTO coffees (name, type, description, ingredients, image, instructions)
    ...
    def self.delete(id)
        results = DB.exec("DELETE FROM coffees WHERE id=#{id};")
        return { "deleted" => true }
    end
